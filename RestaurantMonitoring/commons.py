...
import RestaurantMonitoring.restaurant_montioring_local_settings as local
...
def get_current_restaurant_status_data():
    restautant_status_data=CsvFileReader(local.RESTAURANT_STATUS_CSV_URL,local.RESTAURANT_STATUS_CSV_PATH).get_data()
    restautant_status=[]
    for row in restautant_status_data:
        store_id=row["store_id"].strip()
        status=row["status"].strip()
        timestamp_utc=row["timestamp_utc"].strip()
        restautant_status.append(
            (store_id,status,parse_timestamp(timestamp_utc))
        )
    return restautant_status

#Populate the DB with the latest polling data 
@transaction.atomic
def populate_restaurant_status(apps,schema_editor,time_created):
    progress=0
    restautant_status_obj=[]
    restaurant_data=get_current_restaurant_status_data()
    for status_data in restaurant_data:
        try:
            restaurant=Restaurant.objects.get(store_id=status_data[0])
        except Restaurant.DoesNotExist: 
            restaurant=Restaurant(store_id=status_data[0])
            restaurant.save()
            for i in range(7):
                RestaurantTimings(Restaurant=restaurant,day_of_week=i).save()
        restautant_status_obj.append(RestaurantStatus(
        ...
        if progress%1000==0:
            print(progress)
        progress+=1
    RestaurantStatus.objects.bulk_create(restautant_status_obj)
...
def reverse_migrate(apps, schema_editor,time_created):
    RestaurantStatus.objects.filter(Q(time_created=time_created)).delete()
...
def create_custom_migration(migration_name,time_created):
   ...
   # define the operations
    operations = [migrations.RunPython(
        partial(populate_restaurant_status, time_created=time_created),
    ...
    migration = migrations.Migration(migration_name, "RestaurantMonitoring")
...
def call_migration_by_name(migration_name):
    app_label = 'RestaurantMonitoring'
...
def is_within_business_hours(day_of_week,local_time,restaurant_timing):
    return restaurant_timing.start_time_local <= local_time <= restaurant_timing.end_time_local
...
def fill_Reportdata(time_diff,type):
    ...
    restaurant_statuses = RestaurantStatus.objects.filter(timestamp_utc__range=[start_time, end_time])
    progress=0
    for restaurant in Restaurant.objects.all():
        restaurants_total_operational_time={}
        report_obj=ReportData.objects.get(Restaurant=restaurant)
        for restaurant_status in restaurant_statuses.filter(Restaurant=restaurant):
            day_of_week,local_time=convert_utc_to_local(restaurant_status.timestamp_utc,restaurant.timezone_str)
            restaurant_timing=RestaurantTimings(Restaurant=restaurant,day_of_week=day_of_week)
            restaurants_total_operational_time[day_of_week]=hours_between_times(restaurant_timing.end_time_local,restaurant_timing.start_time_local)
            if is_within_business_hours(day_of_week,local_time,restaurant_timing):
                if type=="hour":
                    if restaurant_status.status=='active':
        ...
        total_operational_time=1+sum(restaurants_total_operational_time.values())
...
def generate_report(report_id):
    ...
    reportData_list=[]
    for restaurant in Restaurant.objects.all():
        if not ReportData.objects.filter(Restaurant=restaurant).exists():
            reportData_list.append(ReportData(Restaurant=restaurant))
    ...
    for data in ReportData.objects.all():
        report_data.append([data.Restaurant.store_id,
